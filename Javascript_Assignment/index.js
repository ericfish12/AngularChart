...
const cutting_string = (str) => {
  ...
  for (let i = 0; i < str.length; i++)
    for (let j = i + 1; j < str.length + 1; j++) target.push(str.slice(i, j));
  return target;
};
...
const isPerfectNumber = (n) => {
  let divisors = [];
  for (let i = 0; i < n; i++) if (n % i == 0) divisors.push(i);
  if (divisors.reduce((ac, n) => ac + n) === n) return true;
  return false;
};
...
const factors_of_integer = (n) => {
  let factors = [];
  for (let i = 0; i < n; i++) if (n % i == 0) factors.push(i);
  return factors;
};
...
const amountTocoins = (amount, coins) => {
  ...
  for (let coin of coins) {
    ...
    if (times > 0) for (let i = 0; i < times; i++) output.push(coin);
    amount = amount % coin;
  }
  return output;
};
...
const numberOfOccurrences = (str) => {
  let occurrences = {};
  for (let i = 0; i < str.length; i++) {
    if (occurrences[str[i]] == undefined) occurrences[str[i]] = 1;
    else occurrences[str[i]]++;
  }
  return occurrences;
};
...
const nPossibleSubset = (array, n) => {
  return allPossibleSubset(array).filter((e) => {
    return e.length >= n;
  });
};
...
const countOccurrences = (str, s) => {
  let count = 0;
  for (let i = 0; i < str.length; i++) if (str[i] === s) count++;
  return count;
};
...
const findLongestSubstring = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    for (let j = i; j < str.length; j++) {
      if (
        str.substring(i, j + 1).includes(str[j + 1]) ||
        j === str.length - 1
      ) {
...
console.log("28: ");
functionPassFunction(aFunction);
